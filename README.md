...
### 飞书集成
- **Channel SDK**：通过 `@larksuiteoapi/node-sdk` 的 `createLarkChannel()` 建立 WebSocket 长连，实时接收消息事件
- **消息收发**：发送/回复/话题线程回复/流式输出（Thinking 卡片逐 chunk 追加）
- **@mention**：结构化 Mention，自动解析 `@user_id` 和 `@all`
- **发送队列**：5 QPS 限流，优先级排序
- **自动重连**：WebSocket 断线自动重连，应用级心跳
- **Remote MCP**：自动注册飞书官方 MCP 工具（文档 CRUD、搜索用户、获取文件等），`tenant_access_token` 自动刷新
- **卡片交互**：支持飞书消息卡片按钮回调
...
系统有四种配置：Agent 定义、LLM Provider、MCP 工具、Skill 技能。全部支持文件热重载。
...
## Instructions
You have access to Feishu cloud services via MCP tools. When asked to write documentation:

1. Create a new Feishu doc using `create-doc` with a clear title
...
飞书 Remote MCP 工具（启动时自动发现，无需配置）：

| 工具 | 说明 |
|------|------|
| `create-doc` / `fetch-doc` / `update-doc` | 文档 CRUD |
| `search-doc` / `list-docs` | 文档搜索与列表 |
| `get-comments` / `add-comments` | 文档评论 |
| `search-user` / `get-user` | 用户搜索与信息 |
| `fetch-file` | 文件内容获取 |
...
| Node.js | ≥ 18 | Channel SDK 插件宿主 | [nodejs.org](https://nodejs.org/) |
...
│       ├── feishu/                # 飞书集成（Channel Bridge + Token Manager）
│       ├── memory/                # 三层记忆系统
│       ├── agent/                 # Agent 生命周期
│       ├── router/                # 消息路由
│       ├── assistant/             # 助理 Agent
